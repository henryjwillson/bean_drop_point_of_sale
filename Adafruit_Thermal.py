...
import sys
import math
from gpiozero import Button as dtr_trigger
...
class Adafruit_Thermal(Serial):
	...
	def __init__(self, *args, **kwargs):
		...
		if self.writeToStdout is False:
			...
			self.dotPrintTime = 0.03 #Original = 0.03
			self.dotFeedTime  = 0.0021 #Original = 0.0021
		else:
	...
	def timeoutWait(self):
		#self.dtrEnabled = False
		if self.dtrEnabled == True:
			print("dtr is enabled")
			print("the dtr_button value is: ", self.dtr_button.value)
			if self.dtr_button.value == 1:
			#	#self.writeToStdout = True
				print("DTR BUTTON PRESSED")
				pass
		else:
			print("Using old timer method...")
			if self.writeToStdout is False:
				while (time.time() - self.resumeTime) < 0: pass

	# Sets up dtr pin Bean Drop Modification
	def set_dtr_pin(self, GPIO_PIN):
		self.dtr_button = dtr_trigger(GPIO_PIN)
		self.dtrEnabled = True
		self.writeBytes(27, 97, 20)#(1<<5))
